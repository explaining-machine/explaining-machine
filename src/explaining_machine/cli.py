...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Explaining-Machine thinks for you about the meanings.",
        epilog="Example:  explaining-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | explaining-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | explaining-machine         #

    $ explaining-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="explaining-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"explaining-machine: {utterance}.")
        syslog.closelog()
